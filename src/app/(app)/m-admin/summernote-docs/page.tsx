...
import { 
    getSummernoteDocsFromMySql, 
    upsertSummernoteDocToMySql, 
    deleteSummernoteDocFromMySql, 
    permanentDeleteSummernoteDocFromMySql 
} from '@/app/actions/summernote-docs';
import {
    Table,
    TableHeader,
    TableBody,
...
import { parseMySqlDateAsUtc } from '@/lib/dateUtils';
...
export default function SummernoteDocsPage(): ReactNode {
    const router = useRouter();
    const { isAdminLoggedIn, adminUser } = useAdminAuth();
    const canEdit = checkClientPermission(adminUser, 'summernote-docs', 'edit');
    const canDelete = checkClientPermission(adminUser, 'summernote-docs', 'delete');
    const canCreate = checkClientPermission(adminUser, 'summernote-docs', 'create');
    const [docs, setDocs] = useState<SummernoteDocument[]>([]);
    ...
    const [trashDocs, setTrashDocs] = useState<SummernoteDocument[]>([]);
    ...
    const fetchDocs = useCallback(async (page: number, search: string, sort: 'newest' | 'oldest' | 'title-asc' | 'title-desc') => {
        ...
        setError(null);
        try {
            const result = await getSummernoteDocsFromMySql({
                page,
    ...
    const handleCreateNew = async () => {
        ...
        const result = await upsertSummernoteDocToMySql(newDoc);
        ...
        router.push(`/m-admin/summernote-docs/${id}`);
    };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            if (deleteTarget.isPermanent) {
                const result = await permanentDeleteSummernoteDocFromMySql(deleteTarget.id);
                if (!result.success) {
                    console.error("Error permanently deleting doc:", result.message);
                    alert("Failed to delete document permanently.");
                }
            } else {
                const result = await deleteSummernoteDocFromMySql(deleteTarget.id);
                if (!result.success) {
        ...
        <div className="min-h-full bg-background/30 p-2 sm:p-4 w-full overflow-x-hidden relative">
            ...
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Manage all your Summernote internal documents.</p>
                </div>
                                            ...
                                            >
                                                ...
                                                <TableCell className="font-medium">
                                                    ...
                                                    <span 
                                                        ...
                                                        onClick={() => canEdit ? router.push(`/m-admin/summernote-docs/${doc.id}`) : window.open(`/docs/summernote/view/${doc.id}`, '_blank')}
                                                    >
                                                        {doc.title}
                                                    </span>
                                                    </div>
                                                ...
                                                <TableCell className="text-right">
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            {canEdit && (
                                                                <DropdownMenuItem onClick={() => router.push(`/m-admin/summernote-docs/${doc.id}`)}>
                                                                    <Edit3 className="mr-2 h-4 w-4" /> Edit Content
                                                                </DropdownMenuItem>
                                                            )}
                                                            <DropdownMenuItem onClick={() => window.open(`/docs/summernote/view/${doc.id}`, '_blank')}>
                                            ...
                                            >
                                                <div className="flex items-center gap-4 min-w-0">
                                                    <div className="w-10 h-10 rounded-lg bg-gray-50 flex items-center justify-center group-hover:bg-white transition-colors">
                                                        <FileText className="h-5 w-5 text-gray-400 group-hover:text-amber-500 transition-colors" />
                                                    </div>
                                                ...
                                                <div className="flex items-center gap-1.5 ml-4">
                                                    {canEdit && (
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            className="h-9 px-3 text-amber-600 hover:text-amber-700 hover:bg-amber-50 rounded-lg font-medium transition-all"
