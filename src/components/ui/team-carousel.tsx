  ...
  return (
    ...
    >
      ...
      {title && (
        <h2
          className={cn(
            "font-black uppercase tracking-tight absolute top-6 left-1/2 transform -translate-x-1/2 pointer-events-none whitespace-nowrap z-10",
      ...
      >
        ...
        {showArrows && (
          <>
            <motion.button
              onClick={() => paginate(-1)}
              className="absolute left-2 sm:left-5 top-1/2 transform -translate-y-1/2 bg-black/60 hover:bg-black/80 text-white w-10 h-10 rounded-full flex items-center justify-center z-20 transition-all duration-300 hover:scale-110 shadow-lg border border-white/20"
              whileTap={{ scale: 0.9 }}
            >
            ...
            <motion.button
              onClick={() => paginate(1)}
              className="absolute right-2 sm:right-5 top-1/2 transform -translate-y-1/2 bg-black/60 hover:bg-black/80 text-white w-10 h-10 rounded-full flex items-center justify-center z-20 transition-all duration-300 hover:scale-110 shadow-lg border border-white/20"
              whileTap={{ scale: 0.9 }}
            >
            ...
            {members.map((member, index) => {
              ...
              return (
                <motion.div
                  key={member.id}
                  className={cn(
                    "absolute bg-card border border-border overflow-hidden shadow-2xl cursor-pointer rounded-2xl",
                    cardClassName
                  )}
                  style={{
                ...
                >
                  ...
                  {infoPosition === 'overlay' && (
                    <div
                      className="absolute bottom-0 left-0 right-0 p-4 text-center backdrop-blur-md"
                      style={{
                        background: infoBackground || "linear-gradient(transparent, rgba(0,0,0,0.85))",
                        color: infoTextColor,
                      }}
                    >
                      <h3 className="text-lg font-bold drop-shadow">{member.name}</h3>
                      <p className="text-sm opacity-90 drop-shadow">{member.role}</p>
                    </div>
                  )}
                </motion.div>
              );
      ...
      {infoPosition === 'bottom' && members[currentIndex] && (
        ...
        >
          ...
          >
            ...
            <span
              className="absolute top-full left-1/4 w-1/2 h-0.5 mt-1 rounded-full"
              style={{ background: infoTextColor }}
            />
          </h2>
          <p
            className="text-sm sm:text-base font-semibold opacity-90 uppercase tracking-wider mt-1"
            style={{ color: infoTextColor }}
          >
